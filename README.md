...
- **Fade In / Fade Out**: 최종 결과물 시작/끝에 페이드 적용 (ms 단위)
- **바이노럴 비트**: L채널=기준 주파수, R채널=기준+비트 주파수로 분리 생성 (헤드폰 권장)
...
3. *(선택)* Fade In / Fade Out 시간 입력 (ms, 기본값 0)
4. *(선택)* **바이노럴 비트** 체크 후 비트 주파수 입력 (헤드폰 착용 권장)
   - L채널: 기준 주파수, R채널: 기준 + 비트 주파수
   - mono 원본도 자동으로 stereo로 변환하여 저장
5. *(선택)* **▶ 테스트 재생** 버튼으로 파형 미리 듣기 — **■ 정지**로 즉시 중단 가능
6. 출력 포맷 선택 (WAV / MP3 / MP4)
7. **음원 내보내기** 버튼 클릭 — 기본 파일명은 `{음원명}_{파형}_{Hz}_{offset}dB[_bin{beat}Hz]` 형식으로 자동 입력
